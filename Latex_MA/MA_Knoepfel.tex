...
Probabilistic Latent Semantic Analysis (PLSA) was first introduced by \cite{Hofmann.1999} and proved to be xxx for topic modeling. The main achievement of PLSA was to supplement the theory of Latent Semantic Analysis with a sound statistical foundation and a proper generative model of the data.
PLSA is based on a mixture model and allows the discovery of latent topics from text data \cite[p.~368]{Zhai.2016}.

 "models each word in a document as a sample from a mixture model, where the mixture components are multinomial random variables that can be viewed as representations of “topics.” \cite[p. 994]{Blei.2003}
 
The linear growth in parameters suggests that the model is prone to overfitting and, empirically, overfitting is indeed a serious problem \cite[p. 1001]{Blei.2003}

\begin{itemize}
	\item probability of observing a word: $p_d(w) = \lambda_Bp(w|\theta_b) + (1-\lambda_B)\sum_{j=1}^{k}\pi_{d,j}p(w|\theta_j)$
	\item known: $\lambda_B$, $p(w|\theta_b)$
...
Topic analysis aims to uncover latent topics in text data using unsupervised text mining techniques. A topic is the theme or subject of a text and can become salient at different levels of granularity - the topic of a sentence, a paragraph, a book, an entire library, etc. In a first step, a set number of topics has to be discovered in the text data. In a second step, the extent to which a specific document in the text collection covers the identified topics has to be measured \cite[p.~329--330]{Zhai.2016}.

Bag of words methods, that is we only consider the vocabulary used, but not the position of the word in the document, or the position of the document in the collection.

%Following the notation of \citeauthor{Zhai.2016}, topic mining task is defined as follows:
The input data of a topic model is usually a collection of $N$ text documents, $C = \{ d_1, ... , d_N\}$, as well as a specification of the number of topics $k$ to be identified. As output, the model yields $k$ topics, $\{\theta_1,...,\theta_k\} $, as well as the probability of document $d_i$ covering topic $\theta_j$, denoted by $\pi_{ij}$, with $\sum_{j=1}^{k}\pi_{ij} = 1$. To set up the task, we need to define what exactly constitutes a topic, that is how to define $\theta_j$ \cite[p.~331]{Zhai.2016}.
