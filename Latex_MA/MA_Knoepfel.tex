...
As outlined above, a topic model is a mixture model that consists of several (latent) variables, i.e. word distributions. An estimation of the model's variables is obtained via Maximum Likelihood (ML) estimation. If a mixture model consists of several components, that is more than just one topic, there is no analytical solution to the ML estimation and a numerical optimization algorithm must be employed. The Expectation-Maximization algorithm (EM) is a family of useful algorithms to obtain an ML estimate of mixture models \cite[~p. 359]{Zhai.2016} According to \citet[p. 290]{Hofmann.1999}, EM is the "standard procedure for maximum likelihood estimation in latent variable models." 

Based on the generative model outlined in Chapter \ref{GM}, the likelihood function can be derived. Recall that $p(w | \theta_i)$ is the probability of word $w$ in the word distribution $\theta_i$ and that $\pi_{d,j}$ is the coverage of topic $\theta_j$ in document $d$. When facing the problem of statistical inference, $\pi_{d,j}$ can be thought of as the probability of topic $\theta_i$ generating word $w$. Then, the probability of observing the sequence of words that make up document $d$ is:
...
\noindent with $w$ being a unique word in the vocabulary set $V = \{w_1, w_2, ..., w_M\}$ and $c(w,d)$ being the count of word $w$ in document $d$. If we consider the entire collection of $N$ documents $C=\{d_1,...,d_N\}$, the likelihood function is:

\begin{align}
\label{LL} \text{log} \ p(C|\Lambda) &= \underset{d \in C}{\sum} \ \underset{w \in V}\sum c(w,d) \ \text{log} \left( \sum_{j=1}^{k}\pi_{d,j} \ p(w|\theta_j) \right)
\end{align}

\noindent with $\Lambda$ denoting the unkown parameters $\pi_{d,j}$ and $\theta_j$, $j\in[1,...,k]$.

A few words of explanation. $\sum_{j=1}^{k}\pi_{d,j} \ p(w|\theta_j)$ denotes the probability of word $w$ occurring once in document $d$. It is the probability of observing the word regardless of which topic is used. We assume indpendence in generating each word, hence the probability of the document equals the probability of each word (see Eq. \ref{prob}). S. 342

In Eq. \ref{prob}


The EM algorithm consists of two sequential steps that are iterated over several times. First, in the expectation step (E), posteriori probabilities for the latent variables are computed. Second, in the maximization step (M), the parameters are updated \cite[p. 290]{Hofmann.1999}. In the E-step, the algorithm uses Bayes' rule to infere which topic has been used to generate a word based on a tentative set of parameters \cite[p. 362]{Zhai.2016}
