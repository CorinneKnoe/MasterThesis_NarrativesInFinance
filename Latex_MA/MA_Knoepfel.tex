...
The aim of a background topic is to represent the common words in the text data so that the inferred topics can capture the words of interest. 

Let's look at the behavior of a mixture model to understand the usefulness of a background model. Topics are word distributions with the simplest word distribution being the unigram language model. In this model, we assume that words in a text are generated independently. Thus, the likelihood of a sequence of words is equal to the product of the probability of each word \citep[p. 51--54]{Zhai.2016}. Assume that $\theta_{u}$ is a unigram language model to be inferred from document $d$. Then the ML estimation problem is given by
\begin{align}
\label{ul} & p(d|\theta_{u}) = \underset{w \in V}\prod p(w|\theta_{B})^{c(w,d)}\\
\label{constraintsul} & \text{s. t. } \underset{w \in V}{\sum} \ p(w|\theta_{u}) = 1
\end{align}
the solution to which is \citep[p. 341-343]{Zhai.2016}:
\begin{align}
\label{estul} & \ p(w_i|\hat{\theta}_{u}) = \frac{c(w_i,d)}{|d|}
\end{align}
where $c(w,d)$ is the word count in document $d$ and $|d|$ is the total number of words in document $d$. Eq. \ref{estul} illustrates that the ML estimator of a unigram language model gives each word a probability equal to its relative frequency in the document. 

A unigram language model gives observed words a non-zero probability and zero probability to words that do not show up. Further, it gives a very high probability to words that show up frequently. The more prominent a word, the higher its probability in the word distribution. This allows a unigram language model to perform semantic analysis as it picks up on words that show up frequently in the same document. Such words will have a similar probability due to their co-occurrence and thus a semantic association is assumed, leading to the mining of a topic. However, usually functional words and stop words are used the most frequently in a text and thus will have high probabilities, while content-carrying words will have a much lower probability. A topic can then be completely dominated by uninformative words. Introducing a background language model can help filter out these words \citep[p. 51--54]{Zhai.2016}.  


It is intuitive that a ML estimator tends to assign the highest probabilities to the most common words in oder to maximize the likelihood function.
